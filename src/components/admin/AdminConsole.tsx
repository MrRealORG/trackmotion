...
export function AdminConsole() {
  ...
  const [authEmail, setAuthEmail] = useState("admin@centerface.web.app");
  const [authPass, setAuthPass] = useState("CenterFaceAdmin2026!");
  ...
  const [gateMode, setGateMode] = useState<"email" | "passkey">("email");
  const [passkey, setPasskey] = useState("centerface2026");
      ...
      <div className="flex min-h-dvh flex-col items-center justify-center bg-black px-4 py-12 text-white">
        <div className="w-full max-w-md rounded-[24px] border border-white/10 bg-[#121214] p-8 shadow-2xl">
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <h1 className="mt-5 text-2xl font-bold tracking-tight text-white">Admin Console</h1>
            <p className="mt-2 text-sm text-white/50">
              Firebase Authentication &amp; System Dashboard
            </p>
          </div>

          {/* Mode Switcher */}
          <div className="mt-6 flex rounded-[10px] bg-black/60 p-1 ring-1 ring-white/10">
            <button
              type="button"
              onClick={() => setGateMode("email")}
              className={`flex-1 rounded-[8px] py-1.5 text-xs font-semibold transition ${
                gateMode === "email" ? "bg-white/15 text-white" : "text-white/40 hover:text-white"
              }`}
            >
              Firebase Sign In
            </button>
            <button
              type="button"
              onClick={() => setGateMode("passkey")}
              className={`flex-1 rounded-[8px] py-1.5 text-xs font-semibold transition ${
                gateMode === "passkey" ? "bg-white/15 text-white" : "text-white/40 hover:text-white"
              }`}
            >
              Master Passkey
            </button>
          ...
          <div className="mt-4 rounded-[12px] border border-lock/20 bg-lock/5 p-3.5 text-xs">
            <div className="flex items-center justify-between text-lock font-semibold">
              <span>Admin Credentials</span>
              <span className="font-mono text-[10px] uppercase">Active</span>
            </div>
            <div className="mt-2 space-y-1 font-mono text-[11px] text-white/80">
              <div>Email: <span className="text-white select-all">admin@centerface.web.app</span></div>
              <div>Password: <span className="text-white select-all">CenterFaceAdmin2026!</span></div>
              <div>Passkey: <span className="text-white select-all">centerface2026</span></div>
            </div>
          </div>
          ...
          {gateMode === "email" ? (
            ...
            <form onSubmit={handleEmailLogin} className="mt-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full rounded-[12px] border border-white/15 bg-black/60 px-4 py-2.5 text-sm text-white placeholder:text-white/30 outline-none focus:border-lock"
                />
              </div>
              ...
              {authError && (
                <div className="rounded-[10px] border border-red-500/25 bg-red-500/10 p-3 text-xs text-red-300">
                  <p>{authError}</p>
                </div>
              )}
              ...
              <div className="relative my-4 flex items-center justify-center">
                ...
                <span className="relative bg-[#121214] px-2 text-[10px] uppercase text-white/40">Or</span>
              </div>
            ...
            /* Master Passkey Unlock Form */
            <form onSubmit={handleUnlockWithPasskey} className="mt-6 space-y-4">
              <div>
